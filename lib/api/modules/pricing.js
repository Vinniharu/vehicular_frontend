...
/**
 * The public counterpart of the admin vehicle-category price grid — all 120
 * cells (5 vehicle_particulars document types + 3 number_plate_* types +
 * roadworthiness_express + central_motor_registry, x 12 vehicle categories),
 * each already resolved server-side (category price if set, else the flat
 * fallback price — roadworthiness_express/central_motor_registry have no
 * fallback tier, so an unset cell for either is null). Unauthenticated (GET
 * /pricing/vehicle-categories). Powers the public /pricing calculator and
 * any wizard needing a live category-based quote. A null amount_kobo means
 * genuinely no price is configured anywhere for that cell — render "Contact
 * us for a price", never ₦0.
 * state_id is optional — omitted, resolves through the general tier.
 */
