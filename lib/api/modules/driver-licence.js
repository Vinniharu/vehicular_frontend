...
export async function submitDriverLicenceApplication({
  ...
  // IS sent directly (not derived from the vehicle).
 ...
 * evidence_documents: [{ doc_type, file_url }] }].
 */
export async function submitVehicleParticularsApplication({ vehicle_id, is_commercial_use, items }) {
  return apiFetch("/applications/vehicle-particulars", {
    method: "POST",
    body: { vehicle_id, is_commercial_use, items },
  });
}
