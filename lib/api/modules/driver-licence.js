...
export async function submitDriverLicenceApplication({
  ...
  // getNumberPlateFee for the live category-aware price).
  use_type, previous_owner_details,
  // number_plate_new / number_plate_change_of_ownership only.
...
/**
 * Submit an Electronic Central Motor Registry application
...
/**
 * Live, vehicle-category-aware price for one number_plate_* type
 * (GET /applications/number-plate/fee?application_type&vehicle_id&state_id).
 * Replaces the old flat fee-schedule price + client-side commercial-
 * surcharge math — call this once both a vehicle and a state are selected
 * in the wizard.
 */
export async function getNumberPlateFee({ application_type, vehicle_id, state_id, is_fancy_plate }) {
  const params = new URLSearchParams({
    application_type,
    vehicle_id: String(vehicle_id),
    state_id: String(state_id),
  });
  if (is_fancy_plate) params.set("is_fancy_plate", "true");
