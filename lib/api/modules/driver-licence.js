...
export async function submitDriverLicenceApplication({
  ...
  delivery_address, vehicle_type, vehicle_body_type,
  is_urgent, processing_speed,
}) {
...
/**
 * Submit a renewal/reissue request for one or more vehicle particulars
 * (POST /applications/vehicle-particulars).
 *
 * items is an array of { document_type: string,
 ...
 */
export async function submitVehicleParticularsApplication({ vehicle_id, items, delivery_address, is_urgent, processing_speed }) {
  return apiFetch("/applications/vehicle-particulars", {
    method: "POST",
    body: { vehicle_id, items, delivery_address, is_urgent, processing_speed },
  });
}
 ...
 */
export async function submitCentralMotorRegistryApplication({ vehicle_id, state_id, nin, applicant_email, vehicle_licence, delivery_address, is_urgent, processing_speed }) {
  return apiFetch("/applications/central-motor-registry", {
    method: "POST",
    body: { vehicle_id, state_id, nin, applicant_email, vehicle_licence, delivery_address, is_urgent, processing_speed },
  });
}
...
export async function submitRoadworthinessApplication({
  vehicle_id, bay_id, slot_template_id, booking_date, papers, prior_application_id, delivery_address, is_urgent, processing_speed,
}) {
  return apiFetch("/applications/roadworthiness-express", {
    method: "POST",
    body: { vehicle_id, bay_id, slot_template_id, booking_date, papers, prior_application_id, delivery_address, is_urgent, processing_speed },
  });
}
...
export async function submitVehicleVerificationApplication({
  ...
  chassis_number, customs_duty_certificate, delivery_address, is_urgent, processing_speed,
}) {
  return apiFetch("/applications/vehicle-verification", {
    method: "POST",
    body: { state_id, check_type, plate_number, make, model, year, colour, reason, chassis_number, customs_duty_certificate, delivery_address, is_urgent, processing_speed },
  });
}
...
export async function submitPhysicalConditionInspectionApplication({
  ...
  vehicle_category, location_address,
  preferred_date, preferred_time, reason, supporting_document, delivery_address, is_urgent, processing_speed,
}) {
...
/**
 * Download an applicant's biodata & specification PDF (`GET
 * /applications/{id}/biodata-pdf`) and save it to the browser.
 * Accessible by applicant customer, assigned agent, staff, and admin.
 */
export async function downloadApplicationBiodataPdf(applicationId) {
  const token = getToken();
  const response = await fetch(`${API_BASE}/applications/${applicationId}/biodata-pdf`, {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  if (!response.ok) {
    let errorMsg = `Could not generate the biodata PDF (error ${response.status}).`;
    try {
...
/**
 * Send image verification link for a tinted permit application (POST /agent/applications/{id}/send-verification-link).
 */
export async function sendVerificationLink(applicationId, { verification_link }) {
  return apiFetch(`/agent/applications/${applicationId}/send-verification-link`, {
    method: "POST",
...
/**
 * Upload multiple temporary documents for an application (POST /agent/applications/{id}/upload-temporary-documents).
 */
export async function uploadTemporaryDocuments(applicationId, { documents }) {
  return apiFetch(`/agent/applications/${applicationId}/upload-temporary-documents`, {
    method: "POST",
    body: { documents },
  });
}
