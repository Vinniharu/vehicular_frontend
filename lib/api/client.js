...
function handleMockFallback(endpoint, method, body, reqHeaders) {
  ...
  if (endpoint === "/auth/me" && method === "GET") {
    ...
    if (typeof window !== "undefined") {
      try {
        const storedBiodata = localStorage.getItem("vh_mock_user_biodata");
        if (storedBiodata) {
          user.biodata = JSON.parse(storedBiodata);
        }
      } catch {}
    }
  ...
  if (endpoint === "/customers/me" && method === "GET") {
    ...
    if (typeof window !== "undefined") {
      try {
        const storedBiodata = localStorage.getItem("vh_mock_user_biodata");
        if (storedBiodata) user.biodata = JSON.parse(storedBiodata);
      } catch {}
    }
    setCachedUser(user);
    return { data: user, error: null, status: 200, requestId: reqId };
  ...
  // PUT /customers/me/biodata
  if (endpoint === "/customers/me/biodata" && method === "PUT") {
    if (!token) {
      return { data: null, error: "Unauthorized", status: 401, requestId: reqId };
    }
    const payload = typeof body === "string" ? JSON.parse(body) : body || {};
    ...
    if (typeof window !== "undefined") {
      try {
        const storedBiodata = localStorage.getItem("vh_mock_user_biodata");
        if (storedBiodata) biodata = JSON.parse(storedBiodata);
      } catch {}
    }
    
    // Merge new fields
    biodata = { ...biodata, ...payload, id: biodata.id || Date.now(), user_id: 1, updated_at: new Date().toISOString() };
    
    if (typeof window !== "undefined") {
      localStorage.setItem("vh_mock_user_biodata", JSON.stringify(biodata));
    }
    
    return { data: biodata, error: null, status: 200, requestId: reqId };
  }
  ...
  if (endpoint.match(/^\/applications\/\d+\/reapply$/) && method === "PATCH") {
    ...
    const updatableFields = [
      ...
      "state_of_origin","lga_of_origin","nationality","visa_status","marital_status",
      "residential_address","nin","blood_group","genotype",
...
export async function apiFetch(endpoint, { method = "GET", body, headers = {} } = {}) {
  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_ENABLE_MOCK_FALLBACK === "true") {
      console.info(`[API Fallback Notice] Live server (${API_BASE}) not reachable. Using local simulated endpoints for ${method} ${endpoint}.`);
      return handleMockFallback(endpoint, method, options.body, reqHeaders);
    }
    console.error(`[API Error] Live server (${API_BASE}) unreachable for ${method} ${endpoint}:`, err);
    return {
      data: null,
      error: `Could not reach the server (${API_BASE}). Check your connection and try again.`,
      status: 0,
      requestId: null,
    };
  }
