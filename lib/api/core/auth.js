 ...
 */
export async function authRegister({ first_name, middle_name, last_name, name, email, phone, password, role = "customer", referred_by_code }) {
  return apiFetch("/auth/register", {
    method: "POST",
    body: { first_name, middle_name, last_name, name, email, phone, password, role, referred_by_code },
  });
}
...
/**
 * Authenticate with a Google ID token and return Bearer JWT access token (POST /auth/google).
 */
export async function authGoogleLogin({ token }) {
 ...
 */
export async function authUpdateProfile({ state_id, lga_id, name, phone, first_name, middle_name, last_name }) {
  const result = await apiFetch("/auth/me", {
    method: "PATCH",
    body: { state_id, lga_id, name, phone, first_name, middle_name, last_name },
  });
...
/**
 * Permanently delete customer account (DELETE /auth/me).
 * Supports password (for password-registered users) or confirmation "DELETE" (for OAuth users).
 */
export async function authDeleteAccount({ password, confirmation } = {}) {
  const result = await apiFetch("/auth/me", {
    method: "DELETE",
    body: { password, confirmation },
  });
  if (result.data && (result.status === 200 || result.status === 204)) {
    removeToken();
  }
  return result;
}
