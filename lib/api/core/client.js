...
// Every portal (customer + 5 staff-side portals) shares this one token —
// see getToken/setToken above — but each has its own login route. Maps the
// current pathname to the right one, same prefix logic every portal layout
// already hardcodes independently for its own mount-time auth check.
const PORTAL_LOGIN_ROUTES = [
  ["/staff", "/staff/login"],
  ["/agent", "/agent/login"],
...
function resolvePortalLoginPath(pathname) {
  const match = PORTAL_LOGIN_ROUTES.find(([prefix]) => pathname.startsWith(prefix));
  return match ? match[1] : "/auth/login";
}

let redirectingToLogin = false;

/**
 * Clears the session and sends the user to the login page for whichever
 * portal they're currently in, with ?reason=session_expired so the login
 * page shows its (already-built) "your session expired" banner. Called on
 * any 401 from apiFetch and by useAutoLogout's proactive expiry timer — the
 * two ways a session can end mid-visit.
 *
 * Guards against redirect storms (e.g. several parallel requests all 401ing
 * at once) with a module-level flag, and does nothing if already on a login
 * page. Uses a hard navigation (not next/navigation's router) since this is
 * a plain module function with no component/router context, and a full
 * reload also clears any stale in-memory app state left over from the dead
 * session.
 */
export function redirectToLogin() {
  ...
  const loginPath = resolvePortalLoginPath(pathname);
  const params = new URLSearchParams({ reason: "session_expired" });
  if (loginPath === "/auth/login") {
    // Only the customer login page supports returning to where the user
    // was (app/auth/login/page.jsx's isSafeRedirect) — the shared
    // PortalLoginForm used by the other 5 portals doesn't read this param.
...
function handleMockFallback(endpoint, method, body, reqHeaders) {
  ...
  // 6a. DELETE /auth/me
  if ((endpoint === "/auth/me" || endpoint === "/customers/me") && method === "DELETE") {
    if (!token) {
      return { data: null, error: "Unauthorized", status: 401, requestId: reqId };
    }
    removeToken();
    return { data: { message: "Your account has been deleted successfully." }, error: null, status: 200, requestId: reqId };
  }
  ...
  if (endpoint.startsWith("/admin/metrics/overview") && method === "GET") {
    return {
      data: {
 ...
 */
// A 401 from one of these pre-auth endpoints means "wrong credentials" (or
// a not-yet-authenticated flow), never "your session expired" — routing
// these through redirectToLogin() would wipe the login form's own error
// message and could loop. Matched by exact endpoint path.
...
export async function apiFetch(endpoint, { method = "GET", body, headers = {} } = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      if (status === 401 && !PRE_AUTH_ENDPOINTS.has(endpoint)) {
        redirectToLogin();
      }
      const errorMessage =
        ...
        (status === 429 ? "Too Many Requests. Rate limit exceeded. Please wait a moment." : (status === 413 ? "File is too large to upload. The maximum allowed file size is 10MB." : `Error ${status}: Request failed`));
