...
/**
 * Record ONE Physical Condition Inspection checklist item's rating (+
 * evidence, for items that require it — see PCI_CHECKLIST_SECTIONS,
 * app/modules/driver_licence/router.py). Unlike RWX's result/evidence_url
 * pair, evidence_url may be omitted for rating-only items (ac_function,
 * electronics).
 */
export async function submitPciChecklistItem(applicationId, sectionKey, itemKey, { rating, evidence_url, notes } = {}) {
  return apiFetch(`/agent/applications/${applicationId}/physical-condition-inspection/checklist/items/${sectionKey}/${itemKey}`, {
    method: "POST",
    body: { rating, evidence_url, notes },
  });
}

/**
 * Completeness-gated final submit — 400s listing exactly which items are
 * still missing a rating/evidence, and enforces the grade partial-order
 * check (a grade can't be "good" if anything underneath it isn't). Produces
 * a graded DRAFT (agent_completed), never a released report — only staff
 * final-review can release one.
 */
export async function submitPciChecklist(applicationId, payload) {
  return apiFetch(`/agent/applications/${applicationId}/physical-condition-inspection/checklist/submit`, {
    method: "POST",
    body: payload,
  });
}
