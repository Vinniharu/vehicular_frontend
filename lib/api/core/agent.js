...
/**
 * Download an applicant's biodata and vehicle/service specification as a PDF
 * (`GET /agent/applications/{id}/biodata-pdf`) and save it to the browser.
 */
export async function downloadAgentBiodataPdf(applicationId) {
  const token = getToken();
  const response = await fetch(`${API_BASE}/agent/applications/${applicationId}/biodata-pdf`, {
 ...
 */
export async function sendAgentSupportChatMessage(applicationId, payload) {
  const bodyText = typeof payload === "string" ? payload : payload?.body;
