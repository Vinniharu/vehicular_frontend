...
/**
 * Correct an application's biodata directly — only allowed while
 * driving_school_enrolled_at is unset (see SupportApplicationDetail).
 * Patch-style: only send the fields you're changing. Does not change
 * application.status. (`PATCH /support/applications/{id}`)
 */
export async function editSupportApplication(applicationId, payload) {
  return apiFetch(`/support/applications/${applicationId}`, {
    method: "PATCH",
    body: payload,
  });
}
 ...
 * (`GET /support/tickets`)
 */
export async function getSupportTickets({ status, claimed_by_id, scope, page = 1, page_size = 20 } = {}) {
  const params = new URLSearchParams();
  if (status) params.set("status", status);
 ...
 */
export async function adminCreateSupport({ name, email, phone, temp_password, first_name, middle_name, last_name }) {
