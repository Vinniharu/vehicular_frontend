 ...
 */
export async function getStaffQueue({ status, application_type, payment_status, state, lga, staff_id, page = 1, page_size = 20, sort, is_urgent_only } = {}) {
  ...
  if (is_urgent_only) params.set("is_urgent_only", "true");
...
/**
 * Retrieve single application details for staff review (`GET /staff/applications/{id}`).
 */
export async function getStaffApplication(applicationId) {
  const cleanId = String(applicationId).replace(/^app_/, "");
  const res = await apiFetch(`/staff/applications/${cleanId}`, {
    method: "GET",
  });
  if (!res.error) return res;
  return apiFetch(`/applications/${cleanId}`, {
    method: "GET",
  });
}
...
/**
 * Called when the 26-day countdown expires on the frontend. Transitions the
 * application from 'driving_school_enrolled' to 'driving_school_graduation' so
 * it remains visible to staff in the driving school queue while they await the
 * physical certificate to arrive.
 * (`POST /staff/applications/{id}/driving-school-countdown-expired`)
 */
export async function staffMarkDrivingSchoolCountdownExpired(applicationId) {
  return apiFetch(`/staff/applications/${applicationId}/driving-school-countdown-expired`, {
    method: "POST",
  });
}
...
/**
 * Staff SLA Countdown and breach monitor
 * (`GET /staff/deadlines/countdown`)
 */
export async function staffGetSlaCountdown({ sla_status, service_key, search, page = 1, page_size = 20 } = {}) {
  const params = new URLSearchParams();
  ...
  return apiFetch(`/staff/deadlines/countdown?${params.toString()}`, { method: "GET" });
}

/**
 * Total aggregate counts across all queue categories (`GET /staff/metrics/counts`).
 */
export async function getStaffCounts() {
  return apiFetch("/staff/metrics/counts", { method: "GET" });
}
