...
/**
 * Spare Parts — dealer vetting queue. GET /staff/dealers
 */
export async function staffListDealers({ status_filter } = {}) {
  const params = new URLSearchParams();
  if (status_filter) params.set("status_filter", status_filter);
  const qs = params.toString();
  return apiFetch(`/staff/dealers${qs ? `?${qs}` : ""}`, { method: "GET" });
}

export async function staffApproveDealer(dealerId) {
  return apiFetch(`/staff/dealers/${dealerId}/approve`, { method: "POST" });
}

export async function staffRejectDealer(dealerId, { reason } = {}) {
  return apiFetch(`/staff/dealers/${dealerId}/reject`, { method: "POST", body: { reason } });
}

export async function staffSuspendDealer(dealerId) {
  return apiFetch(`/staff/dealers/${dealerId}/suspend`, { method: "POST" });
}

export async function staffReinstateDealer(dealerId) {
  return apiFetch(`/staff/dealers/${dealerId}/reinstate`, { method: "POST" });
}

/**
 * Spare Parts — dispute adjudication queue. GET /staff/spare-parts/disputes
 */
export async function staffListSparePartDisputes({ status_filter } = {}) {
  const params = new URLSearchParams();
  if (status_filter) params.set("status_filter", status_filter);
  const qs = params.toString();
  return apiFetch(`/staff/spare-parts/disputes${qs ? `?${qs}` : ""}`, { method: "GET" });
}

/**
 * outcome: 'full_refund' | 'partial_refund' | 'release_to_dealer'.
 * refund_amount_kobo is required (and must be strictly between 0 and the
 * full escrow amount) only for 'partial_refund'.
 * POST /staff/spare-parts/disputes/{disputeId}/resolve
 */
export async function staffResolveSparePartDispute(disputeId, { outcome, refund_amount_kobo, note } = {}) {
  return apiFetch(`/staff/spare-parts/disputes/${disputeId}/resolve`, {
    method: "POST",
    body: { outcome, refund_amount_kobo, note },
  });
}
