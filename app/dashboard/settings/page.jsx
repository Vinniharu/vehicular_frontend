...
import {
  ...
  MessageCircle, Send, Gift, Copy, Check, Users,
} from "lucide-react";
import {
  authGetMe, authUpdateProfile, authChangePassword, getReferenceStates,
  getReferenceLgas, getCachedUser, createCustomerSupportTicket, getReferralDashboard,
} from "@/lib/api";
...
function koboToNaira(kobo) {
  return ((kobo || 0) / 100).toLocaleString("en-NG", { style: "currency", currency: "NGN" });
}
...
export default function SettingsPage() {
  ...
  const handleCopyReferralLink = async () => {
    ...
    try {
      ...
      showToast("error", "Could not copy the link. Please copy it manually.");
    }
  };
      ...
      <div className="bg-white rounded-2xl border border-[#E5E5E5] overflow-hidden">
        <div className="px-6 sm:px-8 py-5 border-b border-slate-100">
          <h2 className="font-display text-base font-semibold text-[#111111] flex items-center gap-2">
            <Gift className="h-4 w-4" style={{ color: BRAND }} /> Referrals
          </h2>
          <p className="text-sm text-slate-500 mt-0.5">Share your code — earn a reward every time someone you referred pays for a service.</p>
        </div>
        {loadingReferral ? (
          ...
          <div className="px-6 sm:px-8 py-6 space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input type="text" readOnly value={referralLink} className={`${inputCls} font-mono text-[13px]`} />
                <button
                  type="button"
                  onClick={handleCopyReferralLink}
                  className="inline-flex items-center gap-2 shrink-0 px-4 py-2.5 rounded-xl text-sm font-semibold border border-[#E5E5E5] bg-slate-50 hover:bg-slate-100 text-slate-700 transition-colors"
                >
