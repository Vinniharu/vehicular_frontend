...
export default function SupportApplicationsPage() {
  ...
  const loadData = async (isRefresh = false) => {
    ...
    setError(null);
    const res = await getSupportApplications({
      page,
      page_size: pageSize,
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600 max-w-2xl">
              Every application with its customer, assigned staff, and assigned agent — agent details
              are shown by id/state/LGA only, not name or contact info.
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Filter by ID, customer, LGA, or staff..."
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center space-y-2">
            ...
            <p className="text-sm text-slate-500 max-w-sm mx-auto">No applications match your current search or filters.</p>
          </div>
        ) : (
                    ...
                    <td className="px-4 py-3.5">
                      {app.assigned_staff ? (
                        <span className="flex items-center gap-1.5 text-[12px] font-medium text-slate-700">
                          ...
                          <span className="truncate">{app.assigned_staff.name}</span>
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3.5">
                      {app.assigned_agent ? (
                        <span className="flex items-center gap-1.5 text-[11.5px] text-slate-500">
                          ...
                          #{app.assigned_agent.agent_id} · {app.assigned_agent.lga}
                        </span>
                      ) : (
                        <span className="text-[12px] text-slate-400">—</span>
                      )}
